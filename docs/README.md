...
**Author:** [Yunhui Gao](https://github.com/Yunhui-Gao) (gao-yh17@mails.tsinghua.edu.cn)
...
For the specific type of problem given by Eq. $(0)$, there is a useful property that states the relation between primal optimal solution and the dual optimal solution. By using the property of the conjugate function, i.e., $g=(g^*)^*$, the primal problem can be reformulated as the following saddle-point problem:
$$
...
Suppose the solution to the above problem is $(\mathbf{z}^\star, \mathbf{w}^\star )$, then according to $(0)$, we have
$$
...
Note that the objective function $h$ of $(5)$ is continuously differentiable, and the gradient is given by
$$
...
According to $(0)$ and using the property of conjugate functions, we have
$$
...
That is, $\mathbf{z}^\star$ is the dual optimal solution.

Similarly, for the primal problem, we have
...
Therefore, proving $(0)$ is equivalent to proving that 
$$
